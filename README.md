...
- **Bits drive engraved width**: every element references a **bit** from a per-machine tool library (engraving bits, V-bits, flat and ball end mills); a thin element is machined once down its centreline at the bit's width, so the cut pattern matches the tool
- **Text stroke thickness**: text has its own stroke-thickness control — a fixed width in mm, or *proportional to font size* (so big text gets bolder automatically). Anything wider than the bit is filled with overlapping bit passes and previewed as a solid stroke, so what you see matches what's cut
