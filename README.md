...
- **Label size follows the outline**: the part is defined by the outermost cut shape, not a separately-set label size. Resize the outline on the canvas and the label tracks it; the single-label export and the panel both use the same outermost outline
